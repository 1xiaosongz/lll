...
if __name__ == "__main__":
    ...
    # R_dot_8.sort()
    # #
    R_dot_7, issues_T_1 = simple_extract_4(R_dot_5)
    T_dot_7 = T_dot_5 + issues_T_1
    T_dot_7.sort()
    #
    # T_dot_9, issues_R_3 = simple_extract_1(T_dot_7)
    # R_dot_9 = R_dot_7 + issues_R_3
    # R_dot_9.sort()
    merged = sorted(R_dot_7 + T_dot_7)

    # 找出需要剔除的元素（后一个元素减前一个元素小于2）
    c = {merged[i + 1] for i in range(len(merged) - 1) if merged[i + 1] - merged[i] < 15}

    # 从a和b中剔除c中的元素
    R_dot_7 = [x for x in R_dot_7 if x not in c]
    T_dot_7 = [x for x in T_dot_7 if x not in c]



    R_dot_10, issues_T_2 = simple_extract_4(R_dot_7)
    T_dot_10 = T_dot_7 + issues_T_2
    T_dot_10.sort()
    #


    merged = sorted(R_dot_10 + T_dot_10)  # 使用 + 运算符合并列表

    # 找出需要剔除的元素（前一位而不是后一位）
    c = [merged[i] for i in range(len(merged) - 1) if merged[i + 1] - merged[i] < 20]

    # 从a和b中剔除c中的元素
    R_dot_10 = [x for x in R_dot_10 if x not in c]
    T_dot_10 = [x for x in T_dot_10 if x not in c]
    R_dot_3 = R_dot_10
    T_dot_3 = T_dot_10
