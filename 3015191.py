...
if __name__ == "__main__":
    ...
    c = {merged[i + 1] for i in range(len(merged) - 1) if merged[i + 1] - merged[i] < 100}
    # 从a和b中剔除c中的元素
    ...
    R_dot_8, issues_T_1 = simple_extract_4(R_dot_7)
    T_dot_8 = T_dot_7 + issues_T_1
    T_dot_8.sort()

    T_dot_10, issues_R_3 = simple_extract_1(T_dot_8)
    R_dot_10 = R_dot_8 + issues_R_3
    R_dot_10.sort()
    ...
    R_T = prediction.find_mutual_nearest_rt(R_dot_3,
                                            T_dot_3)  # 手动RT匹配      T  波和  R波离得很近     否则find_mutual_nearest_rt()
    ...
    a, total_percentages = group_and_count_d_arrays(
        CR_matching)  # 用计算出来的手动标点和上位机标点的差值  计算出上位机R点和手动标点的 <50	50 - 100	100 - 150	150 - 200	>200百分比
    print("------------上位机R点----------")
